...
The LayerZero provider watches `JobAssigned` events emitted by `SendUln302`, batches them into Merkle trees, collects BLS attestations through Symbiotic relay sidecars, and submits proofs to the destination DVN. The destination DVN verifies the quorum via Settlement and forwards verification into `ReceiveUln302`.
...
- `contracts/src/examples/TestOApp.sol`
