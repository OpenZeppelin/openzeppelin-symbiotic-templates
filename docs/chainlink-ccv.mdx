...
The 3-validator row matches what Chainlink measured on the staging deployment (299k–312k), so treat the table as representative. Interpolate for your set size and add a **safety margin (recommend 20%)**. The staging config of `400000` covers roughly a 10-validator set — it is a small-set value, not a constant. (Do not benchmark against the unit tests: `SymbioticVerifier.t.sol` stubs `Settlement`, so its gas numbers exclude all of the above.)
