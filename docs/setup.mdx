...
This creates `.env` from `.env.example`, generates BLS keys, and creates relayer keystores.

`make start` will bootstrap missing local files automatically, so `make setup` is mainly for explicit regeneration.
...
`make start` deploys local contracts, generates runtime config under `generated/local/`, and starts the Docker services.
