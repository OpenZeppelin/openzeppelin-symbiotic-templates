...
2. Signers commit to `keccak256(versionTag ‖ messageId)` — a signature is bound to the message and the verifier implementation version. The digest contains **no chain or settlement domain**, so this prevents replay only across *different* version tags or *disjoint* signer sets: two deployments that share both the version tag and signers can replay each other's attestations. Forks MUST mint their own `versionTag` (it is hardcoded as `0x1a75bd93` in both `DeployCCV.s.sol` and `operator/src/provider/chainlink_ccv.rs` — change both together), and operator BLS keys must never be reused across deployments or networks.
