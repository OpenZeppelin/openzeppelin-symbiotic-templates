...
contract LocalDeploy is SymbioticCoreInit {
    ...
    function setupDVN() public returns (IValSetDriver.CrossChainAddress memory) {
        ...
        SymbioticLayerZeroDVN dvn = new SymbioticLayerZeroDVN(
            address(settlements.get(block.chainid)),
            DVN_BASE_FEE
        );
    ...
    function logAndDumpRelayContracts() public {
        ...
        vm.writeJson(finalJson, "devnet/deploy-data/relay_contracts.json");
    }

    function logAndDumpDVNContracts() public {
        console.log("DVN contracts:");
        for (uint256 i; i < dvns.length(); ++i) {
            (uint256 chainId, address dvn) = dvns.at(i);
            console.log("   DVN (chainId:", chainId, "):", dvn);
        }

        string memory obj = "dvnContracts";

        string[] memory dvnsData = new string[](dvns.length());
        for (uint256 i; i < dvns.length(); ++i) {
            (uint256 chainId, address dvn) = dvns.at(i);
            vm.serializeUint("dvn", "chainId", chainId);
            string memory dvnData = vm.serializeAddress("dvn", "addr", dvn);
            dvnsData[i] = dvnData;
        }
        string memory finalJson = vm.serializeString(obj, "dvns", dvnsData);

        vm.writeJson(finalJson, "devnet/deploy-data/dvn_contracts.json");
    }
